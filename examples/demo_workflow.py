...
from pptx_translator.translator import ArgosTranslateBackend
...
def main() -> None:
    ...
    run_translation(
        ...
        source_lang="de",
        target_lang="en",
        translator=ArgosTranslateBackend(auto_install_package=False),
        dry_run=True,
        dry_run_json_path=DRY_JSON,
    )
    print(f"Wrote dry-run JSON: {DRY_JSON}")

    print("Testing one sentence with Argos backend before slide translation...")
    backend = ArgosTranslateBackend(auto_install_package=False)
    print("Result:", backend.translate_text("Guten Morgen", source_lang="de", target_lang="en"))

    print("Running full translation...")
    run_translation(
        input_path=SAMPLE_IN,
